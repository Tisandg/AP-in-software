...
(:action assign_tester_to_task
  ...
  :precondition (and
    (role-is ?testerman ?free) (not(assigned-to ?t ?testerman))
...
(:action test_task_slow
  ...
  :precondition (and
    (not(role-is ?testerman ?free)) (assigned-to ?t ?testerman)
    (task-in ?t ?previous_s) (not(task-in ?t ?next_s)) (can-finalize ?t ?previous_s)
    (next ?previous_s ?next_s)
  )
  :effect (and
    (role-is ?testerman ?free) (not(assigned-to ?t ?testerman))
    (not(task-in ?t ?previous_s)) (task-in ?t ?next_s)
...
(:action test_task_fast
  ...
  :effect (and
    (role-is ?testerman ?free) (not(assigned-to ?t ?testerman))
    (not(task-in ?t ?previous_s)) (task-in ?t ?next_s)
    (increase (total-cost) (test-fast ?previous_s ?next_s))
  )
)
